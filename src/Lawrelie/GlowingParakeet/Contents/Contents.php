...
class Contents {
    ...
    protected function readProperty_query(): array {
        ...
        list($where, $params) = match (true) {
            $this->isTag => ['lgp_tags LIKE :id OR lgp_tags LIKE :id_prefix', [':id' => \sprintf('%%"%s"%%', $slashedId), ':id_prefix' => \sprintf('%%"%s', $idPrefix)]],
        ...
        try {
            $select = $this->parakeet->db->prepare("SELECT lgp_id, lgp_children, lgp_date FROM lgp_contents WHERE $where ORDER BY lgp_date DESC");
